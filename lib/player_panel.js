...
class PlayerPanel {
  ...
  async handleUnitBuilt(property, builtUnitName) {
    ...
    // Count units at the property location before waiting
    let unitsBefore = this.countUnitsAtCoords(property.coords);

    // Wait for the DOM to update (unit creation happens asynchronously)
    await new Promise(resolve => setTimeout(resolve, 150));

    // Check if a unit was actually created
    let unitsAfter = this.countUnitsAtCoords(property.coords);

    if (unitsAfter > unitsBefore) {
      // Unit was successfully created, deduct funds
      this.liveInfo.funds -= cost;
      this.updateHtml();
    } else {
      console.warn("Ghost production detected: unit not created at", property.coords, "- funds not deducted");
    }
  }

  countUnitsAtCoords(coords) {
    // Check the DOM for units at the specified coordinates
    ...
    for (let unitElement of units) {
      // Parse coordinates from the unit's style attribute
      let left = parseInt(unitElement.style.left);
      let top = parseInt(unitElement.style.top);

      // Convert pixel coordinates to tile coordinates (assuming 16px tiles)
      let unitX = Math.floor(left / 16);
      let unitY = Math.floor(top / 16);
