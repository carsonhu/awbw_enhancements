...
class PlayerPanel {
  ...
  async handleUnitBuilt(property, builtUnitName) {
    ...
    // Count player's total units before
    let unitsBefore = this.playerUnits ? this.playerUnits.length : 0;

    // Wait for DOM update
    await new Promise(resolve => setTimeout(resolve, 200));

    // Trigger a map update to refresh playerUnits
    let gamemap = document.getElementById("gamemap");
    if (gamemap) {
      gamemap.dispatchEvent(new Event('DOMSubtreeModified', { bubbles: true }));
    }

    // Wait a bit more for the update to process
    await new Promise(resolve => setTimeout(resolve, 50));

    // Check if unit count increased
    let unitsAfter = this.playerUnits ? this.playerUnits.length : 0;

    if (unitsAfter > unitsBefore) {
      this.liveInfo.funds -= cost;
      this.updateHtml();
    } else {
      console.warn("Ghost production: unit not created");
    }
  }
