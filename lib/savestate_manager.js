...
<td colspan="4">
    <div class="awbwenhancements-btn-container">
        ...
        <div id="planner-save-tree" class="awbwenhancements-btn awbwenhancements-fixed-btn"
            title="Download the entire snapshot tree to a file.">
            Save Tree
        </div>
        <div id="planner-load-tree" class="awbwenhancements-btn awbwenhancements-fixed-btn"
            title="Load a snapshot tree from a file.">
            Load Tree
        </div>
...
class SavestateManager {
    constructor(controlsTable, baseUrl, savestateInterceptor) {
        ...
        OptionsReader.instance().onOptionsReady((options) => {
            ...
            if (this.options.options_enable_snapshot_tree_view) {
                ...
                this.saveTreeButton = document.getElementById("planner-save-tree");
                this.saveTreeButton.addEventListener("click", this.onSaveTreeClick.bind(this));

                this.loadTreeButton = document.getElementById("planner-load-tree");
                this.loadTreeButton.addEventListener("click", this.onLoadTreeClick.bind(this));
            } else {
    ...
    deserializeTree(json) {
        try {
            ...
            this.treeVisualizer.render();
            console.log("Tree deserialized successfully", this.nodes);
        } catch (e) {
            console.error("Failed to deserialize tree:", e);
            alert("Failed to load tree file. It may be corrupted or invalid.");
        }
    }

    onSaveTreeClick() {
        let json = this.serializeTree();
        let blob = new Blob([json], { type: "application/json" });
        let url = URL.createObjectURL(blob);
        let a = document.createElement("a");
        a.href = url;
        a.download = "awbw_planner_tree.json";
    ...
    onLoadTreeClick() {
        ...
        input.onchange = (e) => {
            ...
            reader.onload = (e) => {
                this.deserializeTree(e.target.result);
            };
...
class TreeVisualizer {
    ...
    render() {
        ...
        let maxX = Math.max(...layout.nodes.map(n => n.x)) + 40; // Add padding

        // Create SVG
        let svg = document.createElementNS("http://www.w3.org/2000/svg", "svg");
        svg.setAttribute("width", maxX);
        svg.setAttribute("height", layout.height + 40);
        ...
        let currentNode = layout.nodes.find(n => n.id === this.manager.currentStateId);
        if (currentNode) {
            // Calculate the position to scroll to (center the current node)
            let scrollLeft = currentNode.x - (this.container.clientWidth / 2);
